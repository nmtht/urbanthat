...
/// <summary>
/// Road surfaces: links (sharp), hubs (selective exterior corner fillets),
...
public sealed class RoadSurfaceGenerator
{
    ...
    public GenerationResult Generate(RhinoDoc doc, RoadNetworkGraph graph)
    {
        ...
        foreach (var eg in edgeData.Values)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Type = "geometry_generation_failed", Severity = IssueSeverity.Error,
        ...
        foreach (var eg in edgeData.Values)
        {
            void Add(string nodeId)
            {
                ...
                { list = new List<EdgeGeom>(); edgesByNode[nodeId] = list; }
                list.Add(eg);
            }
        ...
        foreach (var node in graph.Nodes)
        {
            ...
            if (incident.Count >= 2 && HasAcuteAngle(node, incident))
            {
                graph.Issues.Add(new NetworkIssue
                {
                    Type = "acute_angle_intersection", Severity = IssueSeverity.Warning,
    ...
    {
        if (closed is null || !closed.IsValid || radiusDoc <= _docTolerance * 2) return null;
        try { return IntersectionFillet.FilletExteriorCorners(closed, radiusDoc, _docTolerance); }
        catch { return null; }
    }
    ...
    private double ClampHubRadius(RoadNode node, List<EdgeGeom> incident, double requested)
    {
        ...
        for (var i = 0; i < sorted.Count; i++)
        {
            ...
            var da = DirectionFromNode(a, node.Id);
            var db = DirectionFromNode(b, node.Id);
    ...
    private static double ComputeExtension(double maxWidthAtNode, double edgeLength)
    {
        ...
        return Math.Max(0, Math.Min(ext, cap));
    }
