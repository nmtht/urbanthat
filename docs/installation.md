...
## Самый простой вариант — установить `.rhp` напрямую в Rhino

Готовый `UrbanBridgePlugin.rhp` — это **один файл**, без сторонних DLL и без
установщика. В Rhino выполните `PlugInManager` → **Install…**, выберите этот файл
и подтвердите загрузку. Перезапустите Rhino и выполните `UrbanBridgeStatus`.

## Вариант A — установка скриптом из исходного кода

1. Скачайте исходный код репозитория и распакуйте его в любую папку.
2. Полностью закройте Rhino.
3. Запустите установщик для своей ОС. Если готового `.rhp` ещё нет, скрипт сам собирает Release bundle; для этого в системе нужен .NET SDK 7.x:
...
разрешите её. Если установщик написал, что `dotnet` не найден, установите .NET SDK 7.x
и просто запустите установщик повторно.
