...
1. Скачайте архив релиза UrbanBridge для Rhino 8 **или** исходный код репозитория и распакуйте его в любую папку.
2. Полностью закройте Rhino.
3. Запустите установщик для своей ОС. Если готового `.rhp` рядом нет, скрипт сам собирает Release bundle; для этого в системе нужен .NET SDK 7.x:
...
и просто запустите установщик повторно. Если команда не найдена, в Rhino выполните `PlugInManager` →
